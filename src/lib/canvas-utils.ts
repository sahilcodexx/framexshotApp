...
export function createHighQualityCanvas(options: RenderOptions): HTMLCanvasElement {
  ...
  if (totalPadding === 0) {
    ...
    // Inner shadow: clipped to the image's rounded rect, so the dark
    // gradient only darkens the inside edge of the screenshot (near the
    // right and bottom) instead of bleeding out into the padding area.
    // Keeps the canvas at exactly screenshot + padding while the image
    // still reads as "lifted" off the background.
    ctx.save();
    ctx.beginPath();
    ctx.roundRect(paddingLeft, paddingTop, image.width, image.height, borderRadius);
    ctx.closePath();
    ctx.clip();
    ctx.shadowColor = `rgba(0, 0, 0, ${shadow.opacity / 100})`;
    ctx.shadowBlur = shadow.blur;
    ...
    ctx.fillRect(paddingLeft, paddingTop, image.width, image.height);
    ctx.restore();
  }
