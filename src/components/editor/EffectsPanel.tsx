...
interface EffectsPanelProps {
  ...
  onResetPadding?: () => void;
  /**
   * When true, only the shadow sliders (blur / offsetX / offsetY / opacity)
   * are rendered. Used in the compact slot inside ShadowPresets.
   */
  shadowOnly?: boolean;
  /**
   * When true, the Shadow section is hidden entirely.
   * Used in the main Effects panel to avoid duplicating the shadow UI.
   */
  hideShadow?: boolean;
}
...
export const EffectsPanel = memo(function EffectsPanel({
  ...
  // shadowOnly mode: render only the shadow blur/offset sliders
  if (shadowOnly) {
    return (
